...
对每个技能逐一评估以下指标，加权求和得出总分（21-100 分）：

1. **使用频率（30 分）**：高（每天/每周）/ 中（每月）/ 低（几个月一次）/ 零（装了就没用过）
2. **必要性（25 分）**：不可替代 / 有替代方案 / 可有可无
3. **当前相关性（20 分）**：与当前业务方向匹配 / 部分匹配 / 已不相关
4. **维护状态（10 分）**：近期有更新 / 偶尔更新 / 长期未更新 / 已废弃
5. **独特价值（15 分）**：提供独有能力 / 与其他技能重叠 / 可被通用能力替代
...
| **卸载** | 综合评分 21-49 分；零使用且无独有价值；或与其他技能高度重叠；或已不相关且长期未更新 | 直接卸载 |
